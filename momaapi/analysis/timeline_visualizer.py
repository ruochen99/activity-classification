...
#       self.palette[num_colors] = distinctipy.get_colors(num_colors, pastel_factor=0.5)
...
#
#     # draw sact
#     for i, (interval_sact, label_sact, id_color) in enumerate(zip(intervals_sact, labels_sact, ids_color)):
#       ax.fill_between(interval_sact, 0, 1, color=lighten(colors_sact[id_color]), linewidth=0)
#       ax.vlines(interval_sact, 0, 1.1, color=colors_sact[id_color], linestyles='dashed')
#       ax.plot(interval_sact, [1, 1], '-', color=colors_sact[id_color], label=label_sact)
...
#       ax.fill_between(interval, -0.1, 1.2, color='firebrick', linewidth=0, alpha=0.3)
#
#     # draw hoi
#     ax.plot(times_hoi, [0]*len(times_hoi), '|', color='black', markerfacecolor='w')
#     if index_hoi is not None:
#       time = times_hoi[index_hoi]
#       ax.plot(interval_video, [-0.2, -0.2], '-', color='lightgray', linewidth=4, solid_capstyle='round')
#       ax.plot([interval_video[0], time], [-0.2, -0.2], '-', color='firebrick', linewidth=4, solid_capstyle='round')
#       ax.plot(time, -0.2, marker='o', color='tab:red', markersize=10)
#       ax.text(interval_video[1], -0.2, time.strftime('  %M:%S'), color='firebrick', va='center')
#
