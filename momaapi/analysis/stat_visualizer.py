...
#         distributions[key] = self.moma.distributions_train[key]+self.moma.distributions_val[key]
#         hues[key] = ['train']*len(self.moma.distributions_train[key])+['val']*len(self.moma.distributions_val[key])
...
#       cnames = self.moma.get_taxonomy(key)+self.moma.get_taxonomy(key) if with_split else self.moma.get_taxonomy(key)
...
#       sns.barplot(x=cnames, y=counts, hue=hue, ci=None, ax=ax, log=True, color=color, palette=palette)
